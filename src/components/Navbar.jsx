...
import { Zap, History, Volume2, VolumeX, Globe } from 'lucide-react';
import { TRANSLATIONS } from '../data/translations';

export default function Navbar({ historyCount, onOpenHistory, soundEnabled, onToggleSound, lang, onToggleLang }) {
  const t = TRANSLATIONS[lang] || TRANSLATIONS.RU;

  return (
    <nav className="w-full flex items-center justify-between py-6 px-4 md:px-0 border-b border-[#1F1F1F]/60 mb-8">
      {/* Brand & Badge */}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="px-3.5 py-2 rounded-2xl bg-[#0A0A0A] hover:bg-[#111111] border border-[#1F1F1F] text-zinc-300 hover:text-white transition-all text-xs font-medium flex items-center gap-2 active:scale-95 group cursor-pointer"
        >
